 ...
 */
const BRUTE_FORCE_METHODS = false;
const TEST_NEW = false;
/**
...
trait TaintednessBaseVisitor {
    ...
    protected static $methodByNameCache = null;
...
//				return $taint;
//			}
//            // For any function inside vendor and is not explicitly marked, set params to preserve CUSTOM1_TAINT
//            $definingFile = $trialFunc->getFileRef()->getFile();
//            $excludeAnalysisRegex = Config::getValue('__exclude_analysis_regex');
//            if ($excludeAnalysisRegex!==null &&
//                preg_match($excludeAnalysisRegex, $definingFile) !== 0
//                && $trialFunc->hasReturn()
//                && $trialFunc->getNumberOfParameters() > 0
//                && $trialFunc instanceof Func) {
//                $overallFlags = ( SecurityCheckPlugin::NO_TAINT & SecurityCheckPlugin::FUNCTION_FLAGS ) | SecurityCheckPlugin::NO_OVERRIDE;
//                $taint = new FunctionTaintedness( new Taintedness( SecurityCheckPlugin::NO_TAINT & ~$overallFlags ) );
//                $taint->addOverallFlags($overallFlags);
//                $argCount = $trialFunc->getNumberOfParameters();
//                for ($i = 0; $i < $argCount; $i++) {
//                    $val = SecurityCheckPlugin::CUSTOM1_TAINT;
//                    $paramFlags = ( $val & SecurityCheckPlugin::FUNCTION_FLAGS ) | SecurityCheckPlugin::NO_OVERRIDE;
//                    $pTaint = new Taintedness( $val & ~$paramFlags );
//                    $taint->setParamSinkTaint( $i, $pTaint->withOnly( SecurityCheckPlugin::ALL_EXEC_TAINT ) );
//                    $taint->setParamPreservedTaint(
//                        $i,
//                        $pTaint->without( SecurityCheckPlugin::ALL_EXEC_TAINT )->asPreservedTaintedness()
//                    );
//                    $taint->addParamFlags( $i, $paramFlags );
//                }
//                SecurityCheckPlugin::$builtinFuncTaintCache[(string)$trialFuncName] = $taint;
//                $taint = clone $taint;
//                self::doSetFuncTaint( $func, $taint );
//                if ( !$func->isPHPInternal() ) {
//                    // Caused-by lines are presumably unnecessary for PHP internal stuff.
...
//		return null;
	 ...
	 */
	protected function getFuncsFromNode( Node $node, $caller = __METHOD__, $isPre = false ): iterable {
		...
		if ( $node->kind === \ast\AST_CALL ) {
            $isPre = true;
			try {
                $ret = $this->getCtxN( $node->children['expr'] )->getFunctionFromNode();
                if (!is_array($ret)) {
                    ...
                    if (empty($ret)) {
                        ...
                        $ret = $this->bruteforceFunctions($node);
                    }
                }
                /** @var FunctionInterface $func */
                foreach ($ret as $func) {
                    $funcName = $func->getFQSEN()->__toString();
                    if (in_array($funcName,
                        ["\\updateSettings", "\\loadJavaScriptFile", "\\loadCSSFile", "\\addInlineJavaScript",
                            "\\automanage_attachments_create_directory", "\\ReadDraft", "\\create_control_richedit",
                            "\\template_header", "\\ob_exit", '\\allowedTo', '\\Post', '\\call_integration_hook',
                            '\\is_not_guest', '\\wp_safe_remote_post', '\\wp_safe_remote_get', '\\download_url', '\\wp_get_theme'
                            ,'\\my_setcookie'] )) {
                        return [];
                    }
                }
                ...
                return $this->bruteforceFunctions($node);
				//return [];
			}
		}
        $isStatic = $node->kind === \ast\AST_STATIC_CALL;
        if (BRUTE_FORCE_METHODS && !$isStatic) {
            if (self::$methodByNameCache === null) {
                self::$methodByNameCache = [];
                $methods = $this->code_base->getMethodSet();
                /** @var Method $method */
                foreach ($methods as $method) {
                    $name = $method->getName();
                    if (!isset(self::$methodByNameCache[$name])) {
                        self::$methodByNameCache[$name] = [];
                    }
                    self::$methodByNameCache[$name][] = $method;
                }
            }
            ...
            if (isset(self::$methodByNameCache[$methodName])) {
                return self::$methodByNameCache[$methodName];
            }
            return [];
        }
        handle:
		...
		try {
            $method1  = $this->getCtxN($node)->getMethod($methodName, $isStatic, true);
            if (($method1->getName() === "__call" || $method1->getName() === "__callStatic") && is_string($methodName)) {
                $this->debug($caller, "Method $methodName is __call or __callStatic, trying to find the real method");
                $method2 = $this->tryNameMatch($methodName);
                if ($method2 !== false) {
                    $this->debug($caller, "Found method {$method2->getFQSEN()}");
                    return [$method2];
                }
            }
            return [$method1];
            ...
            if ($methodName instanceof Node) {
                return $this->bruteforceMethods($node);
            }
    ...
    private function bruteforceMethods($node) {
        $file = $this->context->getFile();
        $regex = Config::getValue('__exclude_analysis_regex');
        if ($regex !== null && preg_match($regex, $file) !== 0) {
            return [];
        }
        $this->debug(__METHOD__, "Bruteforcing methods");
        //return [];
        ...
        $argNames = [];
        foreach ($args as $arg) {
            // If arg is var, extract its name
            ...
            $methods = $this->code_base->getMethodSet();
            $argCount = count($args);
            $possibleMethods = $methods->filter(/**
             * @param Method $method
             * @return bool
             */ function ($method) use ($argCount, $argNames) {
                $ret = !$method->isPHPInternal();
                $ret = $ret && $method->getNumberOfRequiredParameters() <= $argCount && $method->getNumberOfParameters() >= $argCount;
                $file = $method->getFileRef()->getfile();
                $regex = Config::getValue('__exclude_analysis_regex');
                if ($regex !== null && preg_match($regex, $file) !== 0) {
                    return false;
                }
                // For each parameter, check whether the corresponding argument name is a substring of the parameter name
                if ($ret && false) {
                    ...
                    foreach ($params as $i => $param) {
                        ...
                        $argName = $argNames[$i];
                        if ($argName ===null ||
                            (stripos($param->getName(), $argName) !== false ||
                            stripos($argName, $param->getName()) !== false)) {
                            $matchCount++;
                        }
                    }
                    if ($matchCount === 0) {
            ...
            if (count($possibleMethods) > 10) {
                return [];
            }
            return $possibleMethods->toArray();
    }

    private function bruteforceFunctions($node) {
        $this->debug(__METHOD__, "Bruteforcing functions");
        $args = $node->children['args']->children;
        if (count($args) === 0) {
            return [];
        }
        ...
        foreach ($args as $arg) {
            ...
            $argTaint = $this->getTaintednessNode( $arg );
            if (!$argTaint->getTaintedness()->isSafe()) {
                $safe = false;
            }
        }
        if (!$safe) {
            $funcs = $this->code_base->getFunctionMap()->valueSet();
            $argCount = count($args);
            $possibleFuncs = $funcs->filter(/**
             * @param Func $func
             * @return bool
             */ function ($func) use ($argCount) {
                 $ret = !$func->isPHPInternal();
                 $ret = $ret && $func->getNumberOfRequiredParameters() <= $argCount && $func->getNumberOfParameters() >= $argCount;
                return $ret;
            });

            return $possibleFuncs->toArray();
        }
        return [];
    }

    private function tryNameMatch($methodName)
    {
        if (self::$methodByNameCache === null) {
            self::$methodByNameCache = [];
            $methods = $this->code_base->getMethodSet();
        ...
        //$methods = $this->code_base->getMethodSetByName($methodName);
        $methods = self::$methodByNameCache[$methodName] ?? [];
        if (count($methods) === 1) {
            return $methods[0];
        }
	...
	protected function markAllDependentMethodsExec(
        ...
        if (strlen($taint->toShortString()) >= 8000) {
            return;
        }
        ...
        if (isset($var->safePath) && $var->safePath) {
            $varLinks = $varLinks->withOnlyFlags( SecurityCheckPlugin::CUSTOM1_TAINT | SecurityCheckPlugin::CUSTOM1_TAINT );
        }
		...
		foreach ( $taint->decomposeForLinks( $varLinks ) as [ $curLinks, $curTaint ] ) {
			...
			foreach ( $curLinks as $method ) {
                if ( isset( $processedMethods[$method->getFQSEN()->__toString()] ) ) {
                    continue;
                }
				...
				foreach ( $paramInfo->getParams() as $i => $paramOffsets ) {
					...
					$this->debug( __METHOD__, "Setting method {$method->getName()} arg $i as $taint due to dependency on {$var->getName()}" );
				}
                ...
                $processedMethods[$method->getFQSEN()->__toString()] = true;
			}
		}
	...
	public function maybeEmitIssue(
		...
		$rhsIsUnknown = $rhsTaint->has( SecurityCheckPlugin::UNKNOWN_TAINT ) && $rhsTaint->without( SecurityCheckPlugin::UNKNOWN_TAINT )->isSafe();
	...
	public function isIssueSuppressedOrFalsePositive( Taintedness $lhsTaint ): bool {
		...
		foreach ( $issues as [ $issueType ] ) {
            ...
            if ($issueType !== 'SecurityCheck-CUSTOM1' && $issueType !== 'SecurityCheck-CUSTOM2') {
                return true;
            }
		}
	...
	public function handleMethodCall(
        ...
        if ($funcName->getName() === 'sprintf') {
            foreach ($args as $i=>$arg) {
                if (isset($arg->marked) && $arg->marked === true) {
                    return TaintednessWithError::newEmpty();
                }
                if ($i === 0) {
                    if ($arg instanceof Node && $arg->kind === \ast\AST_CLASS_CONST) {
                        try {
                            $const = $this->getCtxN($arg)->getClassConst();
                    ...
                    if (is_string($arg)) {
                        $firstPos = strpos($arg, "%s");
                        $startWithHttp = strpos($arg, "http://") !== 0;
                        $startWithHttps = strpos($arg, "https://") !== 0;
                        if ( ($firstPos !== 8 && $startWithHttps) || ($firstPos !== 7 && $startWithHttp)) {
                            return TaintednessWithError::newEmpty();
                        }
                    }
                }
		...
		foreach ( $args as $i => $argument ) {
            ...
            if ($argument->kind === \ast\AST_ENCAPS_LIST) {
                $firstChild = $argument->children[0];
                if (is_string($firstChild) && strpos($firstChild, 'http') === false) {
                    $argTaintWithError = TaintednessWithError::newEmpty();
                }
            }
            if (isset($argument->marked) && $argument->marked === true) {
                $argTaintWithError = TaintednessWithError::newEmpty();
                $paramSinkTaint = Taintedness::newSafe();
            }
			$curArgTaintedness = $argTaintWithError->getTaintedness();
            //$this->debug(__METHOD__, "Argument taint: " . $curArgTaintedness->toShortString());
            ...
            $preserveArgumentsData[$i] = [ $curArgTaintedness, $baseArgError, $argTaintWithError->getMethodLinks() ];
		}
        ...
        $combinedArgLinks = MethodLinks::newEmpty();
        foreach ( $preserveArgumentsData as $i => [ $curArgTaintedness, $baseArgError, $baseArgLinks ] ) {
			if ( $taint->hasParamPreserve( $i ) ) {
                ...
                $curArgLinks = $parTaint->computePreservedLinks( $baseArgLinks );
			} elseif (
                ...
                $curArgLinks = $baseArgLinks->asCollapsed();
			} else {
            ...
            $combinedArgLinks->mergeWith( $curArgLinks );
		}
            ...
            foreach ($args as $i=>$arg) {
                    ...
                    if ($i === 0) {
                        ...
                        if (is_string($arg)) {
                            ...
                            if ( ($firstPos !== 8 && $startWithHttps) || ($firstPos !== 7 && $startWithHttp)) {
                                $combinedArgTaint = Taintedness::newSafe();
                                $callTaintedness = Taintedness::newSafe();
                                $combinedArgErrors = new CausedByLines();
                                $combinedArgLinks = MethodLinks::newEmpty();
                                break;
                            }
                            if (strpos($arg, "%s") === 0 && count($args) >= 2) {
                                if (!array_key_exists(1, $preserveArgumentsData)) {
                                    $combinedArgTaint = Taintedness::newSafe();
                                    $combinedArgErrors = new CausedByLines();
                                    $combinedArgLinks = MethodLinks::newEmpty();
                                    break;
                                }
                                list($curArgTaintedness, $baseArgError, $baseArgLinks) = $preserveArgumentsData[1];
                                $preservedArgTaint = $curArgTaintedness->asCollapsed();
                                $curArgLinks = $baseArgLinks->asCollapsed();
                                $combinedArgTaint = Taintedness::newSafe();
                                $combinedArgErrors = new CausedByLines();
        ...
        $num = count($combinedArgLinks->getMethodAndParamTuples());

        $this->debug(__METHOD__, "Preserved taint for $funcName with $num args");

        return new TaintednessWithError( $callTaintedness, $callError, $combinedArgLinks );
	}

    private function checkSafeString(string $str)
    {
        if ($str === 'http://' || $str === 'https://' || $str === 'tcp://' || $str === 'tls://' || $str === 'udp://') {
            return false;
        }
        ...
        if (isset($result['path']) || isset($result['query']) || isset($result['fragment'])) {
            return true;
        }
     ...
     *
     * @param FunctionInterface $func
     * @param array<array<Taintedness|CausedByLines|MethodLinks>> $preserveArgumentsData Actual taintedness and
     * caused-by lines of each argument. Literal arguments aren't included here.
     * @phan-param array<int,array{0:Taintedness,1:CausedByLines,2:MethodLinks}> $preserveArgumentsData
     * @return TaintednessWithError|null
     * TODO Improve shape inference for links.
     */
    private function getHardcodedPreservedTaintForFunc(
		...
		switch ( ltrim( $func->getName(), '\\' ) ) {
			...
			case 'array_unique':
				...
				$error = $preserveArgumentsData[0][1]->asIntersectedWithTaintedness( $taint );
                $links = $preserveArgumentsData[0][2];
