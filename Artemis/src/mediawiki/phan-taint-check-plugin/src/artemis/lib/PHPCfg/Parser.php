...
class Parser
{
    ...
    protected function parseStmt_Class(Stmt\Class_ $node)
    {
        ...
        $this->block->addChild(new Op\Stmt\Class_(
            $name,
            $node->flags,
    ...
    protected function parseStmt_ClassConst(Stmt\ClassConst $node)
    {
        ...
        foreach ($node->consts as $const) {
            ...
            $this->block->addChild(new Op\Terminal\Const_(
    ...
    protected function parseStmt_ClassMethod(Stmt\ClassMethod $node)
    {
        ...
        $class_method = new Op\Stmt\ClassMethod(
            $func,
            $visibility,
            (bool) $static,
            (bool) $final,
        ...
        $this->block->addChild($class_method);
    ...
    protected function parseStmt_Do(Stmt\Do_ $node)
    {
        ...
        $loopEnd = new Block();
        $this->block->addChild(new Jump($loopBody, $this->mapAttributes($node)));
        ...
        $this->block->addChild(new JumpIf($cond, $loopBody, $loopEnd, $this->mapAttributes($node)));
    ...
    protected function parseStmt_Echo(Stmt\Echo_ $node)
    {
        foreach ($node->exprs as $expr) {
            $this->block->addChild(new Op\Terminal\Echo_(
    ...
    protected function parseStmt_For(Stmt\For_ $node)
    {
        ...
        $this->block->addChild(new Jump($loopInit, $this->mapAttributes($node)));
    ...
    protected function parseStmt_Foreach(Stmt\Foreach_ $node)
    {
        ...
        $this->block->addChild(new Op\Iterator\Reset($iterable, $attrs));

        $loopInit = new Block();
        $loopBody = new Block();
        $loopEnd = new Block();

        $this->block->addChild(new Jump($loopInit, $attrs));
        $loopInit->addParent($this->block);

        $validOp = new Op\Iterator\Valid($iterable, $attrs);
        $loopInit->addChild($validOp);
        $loopInit->addChild(new JumpIf($validOp->result, $loopBody, $loopEnd, $attrs));
        ...
        if ($node->keyVar) {
            $keyOp = new Op\Iterator\Key($iterable, $attrs);
            $this->block->addChild($keyOp);
            $this->block->addChild(new Op\Expr\Assign($this->writeVariable($this->parseExprNode($node->keyVar)), $keyOp->result, $attrs));
        }

        $valueOp = new Op\Iterator\Value($iterable, $node->byRef, $attrs);
        $this->block->addChild($valueOp);
        ...
        if ($node->valueVar instanceof Expr\List_ || $node->valueVar instanceof Expr\Array_) {
            ...
            $this->block->addChild(new Op\Expr\AssignRef($this->writeVariable($this->parseExprNode($node->valueVar)), $valueOp->result, $attrs));
        } else {
            $this->block->addChild(new Op\Expr\Assign($this->writeVariable($this->parseExprNode($node->valueVar)), $valueOp->result, $attrs));
        }
    ...
    protected function parseStmt_Function(Stmt\Function_ $node)
    {
        ...
        $this->parseFunc($func, $node->params, $node->stmts, null);
        $this->block->addChild($function = new Op\Stmt\Function_($func, $this->mapAttributes($node)));
    ...
    protected function parseStmt_Global(Stmt\Global_ $node)
    {
        foreach ($node->vars as $var) {
            ...
            $this->block->addChild(new Op\Terminal\GlobalVar(
    ...
    protected function parseStmt_Goto(Stmt\Goto_ $node)
    {
        ...
        if (isset($this->ctx->labels[$node->name->toString()])) {
            ...
            $this->block->addChild(new Jump($labelBlock, $attributes));
    ...
    protected function parseIf($node, Block $endBlock)
    {
        ...
        $this->block->addChild(new JumpIf($cond, $ifBlock, $elseBlock, $attrs));
        $this->processAssertions($cond, $ifBlock, $elseBlock);

        $this->block = $this->parseNodes($node->stmts, $ifBlock);

        $this->block->addChild(new Jump($endBlock, $attrs));
    ...
    {
        $this->block->addChild(new Op\Terminal\Echo_($this->parseExprNode($node->value), $this->mapAttributes($node)));
    }
    ...
    protected function parseStmt_Interface(Stmt\Interface_ $node)
    {
        ...
        $old = $this->currentClass;
        $this->currentClass = $name;
        $this->block->addChild(new Op\Stmt\Interface_(
            $name,
    ...
    protected function parseStmt_Label(Stmt\Label $node)
    {
        ...
        $this->block->addChild(new Jump($labelBlock, $this->mapAttributes($node)));
        $labelBlock->addParent($this->block);
    ...
    protected function parseStmt_Property(Stmt\Property $node)
    {
        ...
        foreach ($node->props as $prop) {
            ...
            $this->block->addChild(new Op\Stmt\Property(
    ...
    protected function parseStmt_Return(Stmt\Return_ $node)
    {
        ...
        $this->block->addChild(new Op\Terminal\Return_($expr, $this->mapAttributes($node)));
    ...
    protected function parseStmt_Static(Stmt\Static_ $node)
    {
        foreach ($node->vars as $var) {
            ...
            $this->block->addChild(new Op\Terminal\StaticVar(
    ...
    protected function parseStmt_Switch(Stmt\Switch_ $node)
    {
        ...
        foreach ($node->cases as $case) {
            ...
            if ($prevBlock && ! $prevBlock->dead) {
                $prevBlock->addChild(new Jump($ifBlock));
                $ifBlock->addParent($prevBlock);
            }

            if ($case->cond) {
                $caseExpr = $this->parseExprNode($case->cond);
                $cmp = new Op\Expr\BinaryOp\Equal(
                    $this->readVariable($cond),
                    $this->readVariable($caseExpr),
                    $this->mapAttributes($case)
                );
                $this->block->addChild($cmp);

                $elseBlock = new Block();
                $this->block->addChild(new JumpIf($cmp->result, $ifBlock, $elseBlock));
        ...
        if ($prevBlock && ! $prevBlock->dead) {
            $prevBlock->addChild(new Jump($endBlock));
            $endBlock->addParent($prevBlock);
        }

        $this->block->addChild(new Jump($defaultBlock));
    ...
    {
        $this->block->addChild(new Op\Terminal\Throw_(
    ...
    protected function parseStmt_Trait(Stmt\Trait_ $node)
    {
        ...
        $old = $this->currentClass;
        $this->currentClass = $name;
        $this->block->addChild(new Op\Stmt\Trait_(
            $name,
            $this->parseNodes($node->stmts, new Block()),
            $this->mapAttributes($node)
        ));
    ...
    protected function parseStmt_TraitUse(Stmt\TraitUse $node)
    {
        ...
        $this->block->addChild(new TraitUse($traits,$adaptations,$this->mapAttributes($node)));
    }
    ...
    {
        $this->block->addChild(new Op\Terminal\Unset_(
    ...
    protected function parseExprNode($expr)
    {
        ...
        if ($expr instanceof Node\Expr\AssignOp) {
            ...
            $op = new $class($read, $e, $attrs);
            $this->block->addChild($op);
            $this->block->addChild(new Op\Expr\Assign($write, $op->result, $attrs));
        ...
        if ($expr instanceof Node\Expr\BinaryOp) {
            ...
            $op = new $class($left, $right, $this->mapAttributes($expr));
            $this->block->addChild($op);
        ...
        if ($expr instanceof Node\Expr\Cast) {
            ...
            $op = new $class($e, $this->mapAttributes($expr));
            $this->block->addChild($op);
    ...
    protected function parseExpr_ErrorSuppress(Expr\ErrorSuppress $expr)
    {
        ...
        $this->block->addChild(new Jump($block, $attrs));
        $block->addParent($this->block);
        $this->block = $block;
        $result = $this->parseExprNode($expr->expr);
        $end = new Block();
        $this->block->addChild(new Jump($end, $attrs));
    ...
    protected function parseExpr_Exit(Expr\Exit_ $expr)
    {
        ...
        $this->block->addChild(new Op\Terminal\Exit_($e, $this->mapAttributes($expr)));
    ...
    protected function parseExpr_PostDec(Expr\PostDec $expr)
    {
        ...
        $read = $this->readVariable($var);
        $write = $this->writeVariable($var);
        $op = new Op\Expr\BinaryOp\Minus($read, new Operand\Literal(1), $this->mapAttributes($expr));
        $this->block->addChild($op);
        $this->block->addChild(new Op\Expr\Assign($write, $op->result, $this->mapAttributes($expr)));
    ...
    protected function parseExpr_PostInc(Expr\PostInc $expr)
    {
        ...
        $read = $this->readVariable($var);
        $write = $this->writeVariable($var);
        $op = new Op\Expr\BinaryOp\Plus($read, new Operand\Literal(1), $this->mapAttributes($expr));
        $this->block->addChild($op);
        $this->block->addChild(new Op\Expr\Assign($write, $op->result, $this->mapAttributes($expr)));
    ...
    protected function parseExpr_Ternary(Expr\Ternary $expr)
    {
        ...
        $ifVar = new Temporary();
        if ($expr->if) {
            $this->block->addChild(new Op\Expr\Assign(
                $ifVar,
                $this->readVariable($this->parseExprNode($expr->if)),
                $attrs
            ));
        } else {
            $this->block->addChild(new Op\Expr\Assign($ifVar, $cond, $attrs));
        }
        $this->block->addChild(new Jump($endBlock, $attrs));
        $endBlock->addParent($this->block);
        ...
        $assignVar = new Op\Expr\Assign(
            $elseVar,
            $this->readVariable($this->parseExprNode($expr->else)),
            $attrs
        );
        $this->block->addChild($assignVar);
        $this->block->addChild(new Jump($endBlock, $attrs));
        $endBlock->addParent($this->block);
    ...
    {
        $arg = new Op\Expr\ConcatList(
            $this->parseExprList($expr->parts, self::MODE_READ),
            $this->mapAttributes($expr)
        );
        $this->block->addChild($arg);
    ...
    private function compileJumptableSwitch(Stmt\Switch_ $node)
    {
        ...
        foreach ($node->cases as $case) {
            ...
            if ($block && ! $block->dead) {
                // wire up!
                $block->addChild(new Jump($caseBlock));
        ...
        $this->block->addChild(new Op\Stmt\Switch_(
            $cond,
            $cases,
            $targets,
            $defaultBlock,
        ...
        if ($block && ! $block->dead) {
            ...
            $block->addChild(new Jump($endBlock));
    ...
    private function parseShortCircuiting(AstBinaryOp $expr, $isOr)
    {
        ...
        $this->block->addChild(new JumpIf($left, $if, $else));
        ...
        $this->block->addChild($boolCast);
        $this->block->addChild(new Jump($endBlock));
        $endBlock->addParent($this->block);
